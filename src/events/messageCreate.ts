...
export default async function run(client: ShewenyClient, message: Message) {
     ...
     */
		...
		/* ---------------PERMISSIONS--------------- */
		if (command.adminsOnly && !client.admins?.includes(message.author.id)) {return client.managers.commands.emit(COMMAND_EVENTS.userMissingPerm, message, COMMAND_PERMISSIONS.admin);}
		...
		if (message.guild) {
			...
			if (command.userPermissions.length > 0) {
				for (const permission of command.userPermissions) {
					if (!member.permissions.has(permission)) {return client.managers.commands.emit(COMMAND_EVENTS.userMissingPerm, message, permission);}
				}
			}

			if (command.clientPermissions.length > 0) {
				for (const permission of command.clientPermissions) {
					if (!message.guild!.me!.permissions.has(permission)) {return client.managers.commands.emit(COMMAND_EVENTS.clientMissingPerm, message, permission);}
				}
			}
		}
		else {
