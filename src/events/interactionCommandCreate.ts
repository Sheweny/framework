...
export default async function run(client: ShewenyClient, interaction: CommandInteractionExtend) {
	...
	/* ---------------PERMISSIONS--------------- */
	if (command.userPermissions.includes('BOT_ADMIN') && !client.admins?.includes(interaction.user.id)) {
		return client.emit('userMissingPermissions', interaction, 'BOT_ADMIN')
	}
	if (command.userPermissions.length) {
		for (const permission of command.userPermissions) {
			if (!interaction.guild?.members.cache.get(interaction.user.id)!.permissions.has(permission as PermissionString))
				return client.emit('userMissingPermissions', interaction, permission)
		}
	}
	if (command.botPermissions.length) {
		for (const permission of command.botPermissions) {
			if (!interaction.guild!.me!.permissions.has(permission as PermissionString)) return client.emit('botMissingPermissions', interaction, permission)
		}
	}
